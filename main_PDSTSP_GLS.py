...
def solveTSP(ctruck, tmatrix, dist_center):
    ...
    # Setting guided local search for TSP
...
def solvePMSGA(customers, n_drones, time_matrix, dist_center, names):
    ...
    jobsProcessingTime = [(x[0], time_matrix[dist_center][x[1]] * 2) for x in enumerate(customers)]
    jobsProcessingTime.sort(key=lambda x: x[1])
    indexes = list(map(lambda x: x[0], jobsProcessingTime))
    final_names = list(map(lambda x: names[x], indexes))
...
def solvePMS(customers, n_drones, time_matrix, dist_center, names, random_search=False):
    ...
    for x in drone_list:
        drone_status[x] = 'idle'
    jobs = [(x[0], time_matrix[dist_center][x[1]] * 2) for x in enumerate(customers)]
    if random_search:
        import random;
        random.shuffle(jobs)
    else:
    ...
    indexes = list(map(lambda x: x[0], jobs))
    ...
    while remain:
        ...
        for x in drone_status:
            if drone_status[x] != 'busy':
                if jobs:
                    drone_status[x] = 'busy'
                    working_drones.append([(x, jobs[0]), 0.0])
                    jobs.pop(0)
                    jobs_done.append((x, names[indexes.pop(0)]))
        to_pop = list()
